...
from MambaRealSR11_arch import MamberBlock_w
...
class Wavelet_block(nn.Module):
    def __init__(self, dim, wavelet='haar', initialize=True, head=4, drop_rate=0., use_ca=True, use_sa=False):
        ...
        self.conv2 = nn.Conv2d(dim*6, dim*6, 3, padding=1, groups=dim*6)  # dw
        self.conv5 = nn.Conv2d(dim * 6, dim * 6, 5, padding=2, groups=dim * 6)  # dw
        self.conv7 = nn.Conv2d(dim * 6, dim * 6, 7, padding=3, groups=dim * 6)  # dw
        self.act = nn.GELU()
        self.conv3 = nn.Conv2d(dim*6, dim*4, 1)
        # self.VSS = VSSBlock(dim*6)
        self.VSS = MamberBlock_w(dim*6, "WithBias")
        self.ca = ChannelAttention(dim*6, 10)

        self.conv_3 = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
    ...
    def forward(self, x):
        ...
        x1 = self.VSS(self.conv1(dec_x))
        x2 = self.ca(self.conv2(self.act(self.conv1(dec_x))))
        x = self.conv1(dec_x)
        x = self.conv2(x)
        x = self.act(x)
        x = self.conv5(x)
        ...
        # x1 = self.VSS(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
...
class LSKA(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = int(dim/4)
        self.channels = [self.dim, self.dim, self.dim, self.dim]
        self.norm = LayerNormViT(dim, eps=1e-6)
        self.activation = nn.GELU()
        self.conv0h = nn.Conv2d(self.dim, self.dim, kernel_size=(1, 3), stride=(1,1), padding=(0,(3-1)//2), groups=self.dim)
        self.conv0v = nn.Conv2d(self.dim, self.dim, kernel_size=(3, 1), stride=(1,1), padding=((3-1)//2,0), groups=self.dim)
        self.conv_5_h = nn.Conv2d(self.dim, self.dim, kernel_size=(1, 5), stride=(1,1), padding=(0,4), groups=self.dim, dilation=2)
        self.conv_5_v = nn.Conv2d(self.dim, self.dim, kernel_size=(5, 1), stride=(1,1), padding=(4,0), groups=self.dim, dilation=2)
        self.conv_7_h = nn.Conv2d(self.dim, self.dim, kernel_size=(1, 7), stride=(1,1), padding=(0,9), groups=self.dim, dilation=3)
        self.conv_7_v = nn.Conv2d(self.dim, self.dim, kernel_size=(7, 1), stride=(1,1), padding=(9,0), groups=self.dim, dilation=3)
        self.conv1 = nn.Conv2d(dim, dim, 1)
        self.conv1x1_lpl = SeqConv3x3('conv1x1-laplacian', self.dim, self.dim, -1)

    def forward(self, x):
        u = x.clone()
        x1, x2, x3, x4 = torch.split(x, split_size_or_sections=self.channels, dim=1)
        attn_0 = self.conv0h(x1)
        attn_0 = self.activation(attn_0)
        attn_1 = self.conv0v(attn_0)

        attn_5 = self.conv_5_h(x2)
        attn_5 = self.activation(attn_5)
        attn_5 = self.conv_5_v (attn_5)

        attn_7 = self.conv_7_h(x3)
        attn_7 = self.activation(attn_7)
        attn_7 = self.conv_7_v(attn_7)

        attn_l = self.conv1x1_lpl(x4)

        attn = torch.cat([attn_1, attn_5, attn_7,  attn_l], dim=1)
